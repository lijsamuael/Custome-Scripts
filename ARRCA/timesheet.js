...
//filtering tasks based on contract no
// frappe.ui.form.on('Timesheet', {
// 	contract_no: function(frm) {
// 		frappe.call({
// 			method: 'frappe.client.get_list',
// 			args: {
// 				doctype: 'Quotation Items',
// 				filters: {
...
// 				var tasks = response.message.map(item => item.task);
// 				frm.set_df_property('task', 'options', tasks.join('\n'));
// 			}
// 		})
// 	}
// });

//fetching the quantity if the contract no is selected
frappe.ui.form.on('Timesheet', {
	task_name: function(frm) {
		...
		frappe.call({
			...
			args: {
				...
				filters: {
					'parent': frm.doc.contract_no,
					'item': frm.doc.task_name
				},
				fields: ["*"],
			},
			callback: async function(response) {
				console.log("respnoses", response)
				frm.set_value("quantity", response.message[0].quantity);
			}
		})
		}
	}
...
// Material
frappe.ui.form.on("Material Detail", {
